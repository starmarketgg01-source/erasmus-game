// main_fixed.js
// Erasmus Game - corrected collision handling
// - Uses setCollisionByProperty when possible, falls back to setCollisionByExclusion only when needed
// - Removes collisions only on specific ignored tile indices (per-tile), NOT by disabling an entire layer
// - Keeps POI, VILLE, minimap, mobile controls, particles and debug utilities
// - Saves to /mnt/data/main_fixed.js for download
window.onload = function () {
  ...
  // indices you've seen in logs that are *visual* road/bridge tiles but not intended to block
  ...
  function create() {
    ...
    // prefer decorative layer depth tweaks if present
    ...
    if (!spawnPoint) {
      console.warn("spawn_avezzano not found -> fallback to center");
    ...
    // For each collision candidate layer:
    // 1) Try layer.setCollisionByProperty({ collides: true }) -> only tiles flagged in tileset will collide.
    // 2) If that results in zero collidable tiles, fallback to setCollisionByExclusion([-1])
    // 3) In either case, remove per-tile collisions only for the specific IGNORE indices (do not disable whole layer)
    Object.entries(createdLayers).forEach(([name, layer]) => {
      if (!layer || !collisionLayers.includes(name)) return;
      try {
        // step 1: prefer property-based collisions (requires tiles in tileset to have property 'collides': true)
        layer.setCollisionByProperty({ collides: true });
      } catch (err) {
        // ignore if this fails; we'll fallback below
      }

      // count collidable tiles in this layer
      let collidableCount = 0;
      try {
        layer.forEachTile(tile => {
          if (!tile) return;
          if (tile.index === -1) return;
          // Phaser marks tile.collides when flagged; check that first
          if (tile.collides) collidableCount++;
          else if (tile.properties && tile.properties.collides) collidableCount++;
        });
      } catch (e) {
        // layer.forEachTile might throw if layer isn't a tile layer - ignore
      }

      if (collidableCount === 0) {
        // fallback: mark all non-empty tiles collidable
        try {
          layer.setCollisionByExclusion([-1]);
        } catch (err) {}
      }

      // Remove collisions only on the explicit ignore list: PER-TILE disabling
      try {
        layer.forEachTile(tile => {
          if (!tile || tile.index === -1) return;
          if (IGNORE_TILE_INDICES.includes(tile.index)) {
            // disable collisions on this single tile
    ...
    // add collider with player for collidable layers
    Object.entries(createdLayers).forEach(([name, layer]) => {
      if (!layer) return;
      if (collisionLayers.includes(name)) {
        try { this.physics.add.collider(player, layer); } catch(e) { console.warn("collider add fail", name, e); }
      }
    });
    ...
    playerMiniArrow = this.add.triangle(minimapCam.x + miniW/2, minimapCam.y + miniH/2, 0,12, 12,12, 6,0)
    ...
    } else interactionBox.style.display = "none";
  ...
  function update() {
    ...
    else { if (player.anims.currentAnim) { const dir = player.anims.currentAnim.key; if (["up","down","left","right"].includes(dir)) player.anims.play("idle-" + dir, true); } }
    ...
    if (inVille && inVille !== currentVille) { currentVille = inVille; showCityBanner(inVille); }

    // debug blocking - lightweight: logs only when a *real* collidable tile is found
    debugCheckBlocking();
  }
  ...
  function showPressE() {
    if (!document.getElementById("pressE")) {
      ...
      Object.assign(e.style, { position:"absolute", top:"20px", left:"50%", transform:"translateX(-50%)", background:"rgba(0,0,0,0.7)", color:"#fff", padding:"6px 12px", borderRadius:"6px", zIndex:"9999" });
  ...
  function showCityBanner(name) {
    ...
    setTimeout(() => { banner.innerText = name; banner.classList.add("show"); overlay.classList.remove("active"); setTimeout(() => banner.classList.remove("show"), 4000); }, 420);
  }

  // improved debug: only report *real* collidable tiles (non-ignored) near the blocked direction
  function debugCheckBlocking() {
    ...
    for (const c of checks) {
      ...
      if (real.length > 0) {
        ...
        // no real blocking: this is probably a physics collision edge-case or a tile we purposely ignored
